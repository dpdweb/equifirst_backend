...
@php
    // Handle JSON FAQ IDs
    $selectedFaqs = old('faq_ids', isset($record->faq_ids) ? json_decode($record->faq_ids, true) : []);
@endphp
...
<form method="POST" action="{{ route('posts.update', $record) }}" enctype="multipart/form-data">
@csrf
@method('PUT')
...
<div class="mb-3">
    ...
    @php
        $selectedCategories = old(
            'category_ids',
            isset($record) ? $record->categories->pluck('id')->toArray() : []
        );
    @endphp
...
<div class="mb-3">
    ...
    @if ($record->image)
        @php
            ...
            $image = is_array($images) ? ($images['small'] ?? '') : $record->image;
        @endphp

        @if($image)
            <div class="mt-2">
                <img src="{{ asset('storage/' . $image) }}" width="200" class="img-thumbnail">
            </div>
        @endif
    @endif
</div>
...
<div class="mb-3">
    ...
    <input type="text" name="image_title" class="form-control"
        value="{{ old('image_title', $record->image_title) }}">
</div>

<div class="mb-3">
    <label>Image Alt</label>
    <input type="text" name="image_alt" class="form-control"
        value="{{ old('image_alt', $record->image_alt) }}">
        <hr>
</div>
...
<div class="mb-3">
    ...
    <textarea name="meta_description" class="form-control" rows="3">{{ old('meta_description', $record->meta_description) }}</textarea>
</div>
...
<div class="tab-pane fade" id="faqs">
    <div class="mb-3">
        ...
        @php
            // Load saved FAQ IDs for the post or old input (validation)
            $selectedFaqs = old(
                'faq_ids',
                isset($record) ? $record->faqs->pluck('id')->toArray() : []
            );
        @endphp
        <select name="faq_ids[]" id="faq_ids" class="form-control" multiple>
            @foreach ($faqs as $faq)
                <option value="{{ $faq->id }}"
                    {{ in_array($faq->id, $selectedFaqs) ? 'selected' : '' }}>
...
<div class="tab-pane fade" id="tags">
    <div class="mb-3">
        ...
        @php
            $selectedTags = old(
                'tag_ids',
                isset($record) ? $record->tags->pluck('id')->toArray() : []
            );
        @endphp
        @foreach ($tags as $tag)
...
<div class="mb-3">
    ...
    <select name="status"  class="form-select">
            ...
            <option value="unpublish" {{ $record->status=='unpublish' ? 'selected' : '' }}>Unpublish</option>
